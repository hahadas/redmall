...
//                判断是否重合
                if (this.status && this.isEnd == false) {
                    if (this.type !== '1') {		//图片滑动
                        ...
                        let pointJson = this.secretKey ? aesEncrypt(JSON.stringify({x:moveLeftDistance,y:5.0}),this.secretKey):JSON.stringify({x:moveLeftDistance,y:5.0})
						let data = {
                            captchaType:this.captchaType,
                            "pointJson":pointJson,
                            "token":this.backToken
                        }
						this.$http("POST", url.common.checkCaptcha, data).then(result=>{
							let res = result.data
							...
							setTimeout(()=>{
								_this.tipWords = ""
								// _this.$parent.$emit('success', {captchaVerification})
								_this.$emit("success", res.captchaVerification)
							},1000)
						}).catch(()=>{
            ...
            getPictrue(){
				...
				console.log("开始。。。。")
				this.$http("GET", url.common.getCaptcha, data).then(result =>{
