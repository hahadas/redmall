	...
	export default{
		...
		data(){
			return {
				...
				serchParemes: {
					pageSize: 1,
					sortComprehensive: 1, // 综合排序 -1不使用 1使用
					goodsType: -1 // 商品类型 -1全部 1-新品 2-精品
				}
			}
		},
		onLoad(options) {
			...
			if(options.goodsType){
				this.serchParemes.goodsType = options.goodsType;
			}
			if(options.goodsName && options.goodsName !== "undefined"){ // 商品名称模糊搜索
				this.serchParemes.goodsName = options.goodsName;
			}
			if(options.title){
