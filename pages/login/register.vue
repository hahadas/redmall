	...
	<view class="main bg-w">
		...
		<view class="form">
			...
			<view class="btn flex flex-center flex-column">
				...
				<text class="font28 color-purple" @tap="jumpTo('login')">已有账号？马上登录</text>
			</view>
			<!-- #ifdef H5 -->
			<view class="download">
		...
		<view class="flex flex-center flex-column fixed">
			<text class="color-b9 font24" @tap="jumpTo('/pages/notice/agreement?id='+noticeId)">登录注册即表示同意《用户协议及隐私策略》</text>
		</view>
	</view>
</template>
		...
		methods: {
			async onRegister(){
				...
				this.$http("POST", url.login.register, this.form).then(res => {
					if (res.code === 200) {
						this.onLogin({username: this.form.username})
					}
				}).catch(()=>{
					this.loading = false
				})
