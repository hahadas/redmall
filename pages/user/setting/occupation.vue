<template>
	<view class="main">
		<view class="flex flex-between input">
			<input type="text" placeholder="如建筑师、演员" :maxlength="10" v-model="txt" />
			<text class="right bg-base" @click="onAdd">添加</text>
		</view>
		<text class="title" v-if="selectList.length > 0">我的过往职业</text>
		<view class="list">
...
<script>
	...
	export default{
		...
		onLoad(opt) {
			let labelPastOccupation = this.userInfo.labelPastOccupation
			if (labelPastOccupation) {
				labelPastOccupation = labelPastOccupation.split("-")
				this.selectList = labelPastOccupation
			}
			if (opt.labels) {
				...
				labels.forEach((v, i) =>{
					if (v.type === 1) {
						if (labelPastOccupation && labelPastOccupation.includes(v.text)){
							v.select = true
						}
						list.push(v)
					}
		...
		methods:{
			onConfirm(){
				this.loading = true
				this.$http("POST", url.user.editUserInfo, {labelPastOccupation: this.selectList.join("-")}).then(res =>{
					this.$msg(res.data)
